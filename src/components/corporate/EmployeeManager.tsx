                ...
                <div className="flex gap-2 bg-gray-100 rounded-xl p-1">
                    {(['all', 'active', 'pending', 'suspended'] as const).map((status) => (
                        <button
                            ...
                            className={`px-3 py-2 rounded-lg text-sm font-medium transition-all capitalize flex items-center gap-1 ${filterStatus === status
                                    ? 'bg-white text-gray-900 shadow'
                                    : 'text-gray-600 hover:text-gray-900'
                                }`}
                        >
                            {status}
                            <span className={`text-xs px-1.5 py-0.5 rounded-full ${filterStatus === status ? 'bg-gray-100' : 'bg-gray-200'
                                }`}>
                                {statusCounts[status]}
                            </span>
                        </button>
